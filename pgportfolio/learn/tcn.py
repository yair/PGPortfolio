...
#logging.error('Running' if __name__ == '__main__' else 'Importing', Path(__file__).resolve())

# let's do it the fugly hack way
fugly_hack = 0
...
def weightNormConvolution1d(x, num_filters, dilation_rate, filter_size=3, stride=[1],
    ...
    with tf.variable_scope(name):
        ...
        if init:
            ...
            logging.error("initializing weight norm")
...
def TemporalBlock(input_layer, out_channels, filter_size, stride, dilation_rate, counters,
    ...
    logging.error ('TemporalBlock name: ' + name)
    with tf.variable_scope(name):

        # num_filters is the hidden units in TCN
        # which is the number of out channels
        logging.error("Before wNC1d - shape(input_layer)=" + str(tf.shape(input_layer)) + " out_channels=" + str(out_channels))
        logging.error("input_layer's graph -- " + str(input_layer.graph))
        ...
        logging.error("conv1's graph -- " + str(conv1.graph))
        ...
        logging.error("shape(conv1) = " + str(tf.shape(conv1)) + " noise_shape = " + str(noise_shape))
        training_mode = tflearn.get_training_mode()
        if fugly_hack == 0 and training_mode:
            logging.error('We are in training mode, applyting dropout.')
            out1 = tf.nn.dropout(conv1, keep_prob, noise_shape) # <-- crashes
        else:
            logging.error('We are not in training mode. Not applyting dropout.')
            out1 = conv1
        if atten:
        ...
        if use_highway:
            ...
            logging.error("no residual convolution")
...
def TemporalConvNet(input_layer, num_channels, sequence_length, kernel_size=2,
    ...
    counters = {}
    logging.error('TemporalConvNet called. fugly_hack=' + str(fugly_hack));
    for i in range(num_levels):
        #print(i)
        logging.error('Level ' + str(i))
        dilation_size = 2 ** i
